...
if opt.finetune == '' then -- build network from scratch
  ...
  -- encode_net: outputsize: 32, 1024, 4, 4
  ...
  --mengmi: modified to 3d convolution neural net for background stream
  static_net = nn.Sequential()
  static_net:add(nn.View(-1, 1024, 1, 4, 4))
  static_net:add(nn.VolumetricFullConvolution(1024, 1024, 2,1,1))
  static_net:add(nn.VolumetricBatchNormalization(1024)):add(nn.ReLU(true))
  static_net:add(nn.VolumetricFullConvolution(1024, 512, 4,4,4, 2,2,2, 1,1,1))
  static_net:add(nn.VolumetricBatchNormalization(512)):add(nn.ReLU(true))
  static_net:add(nn.VolumetricFullConvolution(512, 256, 4,4,4, 2,2,2, 1,1,1))
  static_net:add(nn.VolumetricBatchNormalization(256)):add(nn.ReLU(true))
  static_net:add(nn.VolumetricFullConvolution(256, 128, 4,4,4, 2,2,2, 1,1,1))
  static_net:add(nn.VolumetricBatchNormalization(128)):add(nn.ReLU(true))
  static_net:add(nn.VolumetricFullConvolution(128,3, 4,4,4, 2,2,2, 1,1,1))
  ...
  local sta_part = nn.Sequential():add(nn.ConcatTable():add(nn.SelectTable(3))                                                                           
                                                       :add(nn.Sequential():add(nn.SelectTable(2))
                                                                           :add(nn.Squeeze())
  ...
  --save the model at the 0 iter
  paths.mkdir('../models/'.. opt.name)
  torch.save('../models/'.. opt.name .. '/iter0_net.t7', net:clearState())
  torch.save('../models/'.. opt.name .. '/iter0_netD.t7', netD:clearState())
  torch.save('../models/'.. opt.name .. '/iter0_history.t7', history)
...
for epoch = 1,opt.niter do -- for each epoch
  for i = 1, math.min(data:size(), opt.ntrain), opt.batchSize do -- for each mini-batch
    ...
    if counter % 100 == 0 then
      ...
      local vis_tab = {}
      for i=1,opt.frameSize do table.insert(vis_tab, vis[{ {}, {}, i, {}, {} }]) end
      disp.image(torch.cat(vis_tab, 3), {win=opt.display_id+4, title=(opt.name .. ' static')})
    ...
    if counter % opt.saveIter == 0 then
      ...
      paths.mkdir('../models/'.. opt.name)
      torch.save('../models/'.. opt.name .. '/iter' .. counter .. '_net.t7', net:clearState())
      torch.save('../models/'.. opt.name .. '/iter' .. counter .. '_netD.t7', netD:clearState())
